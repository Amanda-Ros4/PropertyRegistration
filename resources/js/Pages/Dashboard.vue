...
<template>
    <AppLayout :title="trans('dashboard.title')">
        ...
        <div class="space-y-6 max-w-7xl mx-auto">
            ...
            <!-- Bloco Superior em Flexbox Responsivo (Garante que empilha se a sidebar abrir ou a tela diminuir) -->
            <div class="flex flex-col lg:flex-row gap-6">

                <!-- Caixa principal da esquerda (ocupa flex-1 preenchendo o espaço) -->
                <div
                    class="flex-1 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-green-700 dark:text-green-400 mb-3">
                        {{ trans('dashboard.context') }}
                    </p>
                    <h1 class="text-2xl font-bold mb-2 text-slate-900 dark:text-slate-100">{{ trans('dashboard.welcome')
                        }}</h1>
                    <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{{
                        trans('dashboard.description') }}</p>
                ...
                <!-- Caixa de Resumo Operacional da direita  -->
                <div
                    class="w-full lg:w-[380px] shrink-0 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
                    <p class="text-sm font-semibold text-green-700 dark:text-green-300 mb-2">{{
                        trans('dashboard.operational_summary_title') }}</p>
                    <p class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed mb-5">{{
                        trans('dashboard.operational_summary_description') }}</p>
                    <div class="space-y-3">
                        <div
                            class="rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 px-4 py-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <Link :href="route('people.index')" class="block">
                    <div
                        class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm hover:border-green-300 dark:hover:border-green-700 transition-colors cursor-pointer group">
                        <div class="flex items-center justify-between">
                            ...
                            <div
                                class="w-12 h-12 rounded-xl bg-green-50 dark:bg-green-950/20 flex items-center justify-center group-hover:bg-green-100 dark:group-hover:bg-green-900/30 transition-colors">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{
                                    trans('dashboard.total_properties') }}</p>
                                <p class="text-3xl font-bold text-slate-900 dark:text-slate-100 mt-1">{{
                                    stats.totalProperties }}</p>
                            </div>
                            <div
                                class="w-12 h-12 rounded-xl bg-green-100 dark:bg-green-950/30 flex items-center justify-center group-hover:bg-green-200 dark:group-hover:bg-green-900/30 transition-colors">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div
                    ...
                    <div v-if="recentPeople.length === 0"
                    ...
                    <ul v-else class="divide-y divide-slate-50 dark:divide-slate-800">
                        <li v-for="person in recentPeople" :key="person.id"
                ...
                <div
                    ...
                    <div v-if="recentProperties.length === 0"
                        class="p-8 text-center text-slate-400 dark:text-slate-500 text-sm">
                        {{ trans('dashboard.no_properties') }}
                    </div>
                    <ul v-else class="divide-y divide-slate-50 dark:divide-slate-800">
                        <li v-for="property in recentProperties" :key="property.id"
                            class="flex items-center justify-between px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div>
